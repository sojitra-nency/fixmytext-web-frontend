...
describe('BottomPanel', () => {
  const baseProps = {
    ...
    history: { history: [], handleRestoreOriginal: vi.fn(), handleRestoreResult: vi.fn(), handleClearHistory: vi.fn() },
  ...
  it('shows pipeline steps when present', () => {
    render(<BottomPanel {...baseProps} pipeline={{ steps: [{ label: 'Uppercase', timestamp: 1 }], clearPipeline: vi.fn() }} />)
    fireEvent.click(screen.getByText('Pipeline'))
  ...
  it('shows pipeline badge when steps exist', () => {
    render(<BottomPanel {...baseProps} pipeline={{ steps: [{ label: 'A' }, { label: 'B' }], clearPipeline: vi.fn() }} />)
    // Badge "2" appears on the Pipeline tab button; may also appear in stats
    expect(screen.getAllByText('2').length).toBeGreaterThan(0)
  })

  it('shows gamification stats when provided', () => {
    render(<BottomPanel {...baseProps} gamification={{ xp: 250, discoveredTools: ['a', 'b', 'c'] }} />)
