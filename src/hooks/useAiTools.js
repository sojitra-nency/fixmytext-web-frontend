...
export default function useAiTools(text, setText, setMarkdownMode, setPreviewMode, showAlert, pushHistory) {
    ...
    const handleAcademicStyle    = () => callAi(ENDPOINTS.ACADEMIC_STYLE,     'Academic Style',    'Could not convert to academic style.', 'academic_style')
    const handleCreativeStyle    = () => callAi(ENDPOINTS.CREATIVE_STYLE,     'Creative Style',    'Could not convert to creative style.', 'creative_style')
    const handleTechnicalStyle   = () => callAi(ENDPOINTS.TECHNICAL_STYLE,    'Technical Style',   'Could not convert to technical style.', 'technical_style')
    const handleActiveVoice      = () => callAi(ENDPOINTS.ACTIVE_VOICE,       'Active Voice',      'Could not convert to active voice.', 'active_voice')
    const handleRedundancyRemover = () => callAi(ENDPOINTS.REDUNDANCY_REMOVER, 'Remove Redundancy', 'Could not remove redundancy.', 'redundancy_remover')
    const handleSentenceSplitter = () => callAi(ENDPOINTS.SENTENCE_SPLITTER,  'Split Sentences',   'Could not split sentences.', 'sentence_splitter')
    const handleConciseness      = () => callAi(ENDPOINTS.CONCISENESS,        'Make Concise',      'Could not make text concise.', 'conciseness')
    const handleResumeBullets    = () => callAi(ENDPOINTS.RESUME_BULLETS,     'Resume Bullets',    'Could not generate resume bullets.', 'resume_bullets')
    const handleMeetingNotes     = () => callAi(ENDPOINTS.MEETING_NOTES,      'Meeting Notes',     'Could not generate meeting notes.', 'meeting_notes')
    const handleCoverLetter      = () => callAi(ENDPOINTS.COVER_LETTER,       'Cover Letter',      'Could not generate cover letter.', 'cover_letter')
    const handleOutlineToDraft   = () => callAi(ENDPOINTS.OUTLINE_TO_DRAFT,   'Outline→Draft',     'Could not expand outline.', 'outline_to_draft')
    const handleContinueWriting  = () => callAi(ENDPOINTS.CONTINUE_WRITING,   'Continue Writing',  'Could not continue writing.', 'continue_writing')
    const handleRewriteUnique    = () => callAi(ENDPOINTS.REWRITE_UNIQUE,     'Rewrite Unique',    'Could not rewrite uniquely.', 'rewrite_unique')
    const handleToneAnalyzer     = () => callAi(ENDPOINTS.TONE_ANALYZER,      'Tone Analysis',     'Could not analyze tone.', 'tone_analyzer')

    // New AI Content handlers
    const handleLinkedinPost     = () => callAi(ENDPOINTS.LINKEDIN_POST,      'LinkedIn Post',     'Could not generate LinkedIn post.', 'linkedin_post')
    const handleTwitterThread    = () => callAi(ENDPOINTS.TWITTER_THREAD,     'Twitter Thread',    'Could not generate Twitter thread.', 'twitter_thread')
    const handleInstagramCaption = () => callAi(ENDPOINTS.INSTAGRAM_CAPTION,  'Instagram Caption', 'Could not generate Instagram caption.', 'instagram_caption')
    const handleYoutubeDesc      = () => callAi(ENDPOINTS.YOUTUBE_DESC,       'YouTube Description', 'Could not generate YouTube description.', 'youtube_desc')
    const handleSocialBio        = () => callAi(ENDPOINTS.SOCIAL_BIO,         'Social Bio',        'Could not generate social bio.', 'social_bio')
    const handleProductDesc      = () => callAi(ENDPOINTS.PRODUCT_DESC,       'Product Description', 'Could not generate product description.', 'product_desc')
    const handleCtaGenerator     = () => callAi(ENDPOINTS.CTA_GENERATOR,      'CTAs',              'Could not generate CTAs.', 'cta_generator')
    const handleAdCopy           = () => callAi(ENDPOINTS.AD_COPY,            'Ad Copy',           'Could not generate ad copy.', 'ad_copy')
    const handleLandingHeadline  = () => callAi(ENDPOINTS.LANDING_HEADLINE,   'Landing Headline',  'Could not generate headlines.', 'landing_headline')
    const handleEmailSubject     = () => callAi(ENDPOINTS.EMAIL_SUBJECT,      'Email Subject',     'Could not generate subject lines.', 'email_subject')
    const handleContentIdeas     = () => callAi(ENDPOINTS.CONTENT_IDEAS,      'Content Ideas',     'Could not generate content ideas.', 'content_ideas')
    const handleHookGenerator    = () => callAi(ENDPOINTS.HOOK_GENERATOR,     'Hooks',             'Could not generate hooks.', 'hook_generator')
    const handleAngleGenerator   = () => callAi(ENDPOINTS.ANGLE_GENERATOR,    'Angles',            'Could not generate angles.', 'angle_generator')
    const handleFaqSchema        = () => callAi(ENDPOINTS.FAQ_SCHEMA,         'FAQ Schema',        'Could not generate FAQ schema.', 'faq_schema')

    // New Language handlers
    const handlePosTagger        = () => callAi(ENDPOINTS.POS_TAGGER,         'Parts of Speech',   'Could not tag parts of speech.', 'pos_tagger')
    const handleSentenceType     = () => callAi(ENDPOINTS.SENTENCE_TYPE,      'Sentence Type',     'Could not classify sentences.', 'sentence_type')
    const handleGrammarExplain   = () => callAi(ENDPOINTS.GRAMMAR_EXPLAIN,    'Grammar Explain',   'Could not explain grammar.', 'grammar_explain')
    const handleSynonymFinder    = () => callAi(ENDPOINTS.SYNONYM_FINDER,     'Synonyms',          'Could not find synonyms.', 'synonym_finder')
    const handleAntonymFinder    = () => callAi(ENDPOINTS.ANTONYM_FINDER,     'Antonyms',          'Could not find antonyms.', 'antonym_finder')
    const handleDefineWords      = () => callAi(ENDPOINTS.DEFINE_WORDS,       'Definitions',       'Could not define words.', 'define_words')
    const handleWordPower        = () => callAi(ENDPOINTS.WORD_POWER,         'Power Words',       'Could not enhance words.', 'word_power')
    const handleVocabComplexity  = () => callAi(ENDPOINTS.VOCAB_COMPLEXITY,   'Vocab Complexity',  'Could not analyze vocabulary.', 'vocab_complexity')
    const handleJargonSimplifier = () => callAi(ENDPOINTS.JARGON_SIMPLIFIER,  'Jargon Simplifier', 'Could not simplify jargon.', 'jargon_simplifier')
    const handleFormalityDetector = () => callAi(ENDPOINTS.FORMALITY_DETECTOR, 'Formality',        'Could not detect formality.', 'formality_detector')
    const handleClicheDetector   = () => callAi(ENDPOINTS.CLICHE_DETECTOR,    'Cliche Detector',   'Could not detect cliches.', 'cliche_detector')

    // New Generator AI handlers
    const handleRegexGen         = () => callAi(ENDPOINTS.REGEX_GEN,          'Regex Pattern',     'Could not generate regex.', 'regex_gen')
    const handleWritingPrompt    = () => callAi(ENDPOINTS.WRITING_PROMPT,     'Writing Prompt',    'Could not generate prompt.', 'writing_prompt')
    const handleTeamNameGen      = () => callAi(ENDPOINTS.TEAM_NAME_GEN,     'Team Names',        'Could not generate names.', 'team_name_gen')
    const handleMockApiResponse  = () => callAi(ENDPOINTS.MOCK_API_RESPONSE,  'Mock API',          'Could not generate mock response.', 'mock_api_response')
    ...
    const handleCaesarCipher = async (overrideVal) => {
        ...
        try {
            ...
            setAiResult({ label, result: data.result })
            setPreviewMode('result')
            if (pushHistory) pushHistory(label, original, data.result, { toolId: 'caesar_cipher', toolType: 'select' })
    ...
    const handleRailFenceEnc = async (overrideVal) => {
        ...
        try {
            ...
            const data = await transformText({ endpoint: ENDPOINTS.RAIL_FENCE_ENC, text, rails }).unwrap()
            setAiResult({ label, result: data.result })
            setPreviewMode('result')
            if (pushHistory) pushHistory(label, original, data.result, { toolId: 'rail_fence_enc', toolType: 'select' })
    ...
    const handleRailFenceDec = async (overrideVal) => {
        ...
        try {
            ...
            const data = await transformText({ endpoint: ENDPOINTS.RAIL_FENCE_DEC, text, rails }).unwrap()
            setAiResult({ label, result: data.result })
            setPreviewMode('result')
            if (pushHistory) pushHistory(label, original, data.result, { toolId: 'rail_fence_dec', toolType: 'select' })
    ...
    const handleCurlToCode = async (overrideVal) => {
        ...
        try {
            ...
            const headers = Object.fromEntries(headerMatches.map(m => m[1].split(': ').map(s => s.trim())).filter(p => p.length === 2))
            const dataMatch = text.match(/-d\s+['"]([^'"]+)['"]/) || text.match(/--data\s+['"]([^'"]+)['"]/)
            ...
            if (target === 'javascript') {
                ...
                if (body) lines.push(`response = requests.${method.toLowerCase()}('${url}', headers=${JSON.stringify(headers)}, data='${body}')`)
                else lines.push(`response = requests.${method.toLowerCase()}('${url}', headers=${JSON.stringify(headers)})`)
        ...
        try {
            ...
            const lines = text.split('\n').map(line => {
                ...
                switch (fmt) {
                    ...
                    case 'us': return `${(d.getMonth()+1).toString().padStart(2,'0')}/${d.getDate().toString().padStart(2,'0')}/${d.getFullYear()}`
                    case 'eu': return `${d.getDate().toString().padStart(2,'0')}/${(d.getMonth()+1).toString().padStart(2,'0')}/${d.getFullYear()}`
    ...
    return {
        ...
        caesarShift, setCaesarShift, railCount, setRailCount,
        curlTarget, setCurlTarget, dateFormatType, setDateFormatType,
        // New select tool handlers
        handleCaesarCipher, handleRailFenceEnc, handleRailFenceDec,
        handleCurlToCode, handleDateFormat,
        // New AI Writing handlers
        handleAcademicStyle, handleCreativeStyle, handleTechnicalStyle,
        handleActiveVoice, handleRedundancyRemover, handleSentenceSplitter,
        handleConciseness, handleResumeBullets, handleMeetingNotes,
        handleCoverLetter, handleOutlineToDraft, handleContinueWriting,
        handleRewriteUnique, handleToneAnalyzer,
        // New AI Content handlers
        handleLinkedinPost, handleTwitterThread, handleInstagramCaption,
        handleYoutubeDesc, handleSocialBio, handleProductDesc,
        handleCtaGenerator, handleAdCopy, handleLandingHeadline,
        handleEmailSubject, handleContentIdeas, handleHookGenerator,
        handleAngleGenerator, handleFaqSchema,
        // New Language handlers
        handlePosTagger, handleSentenceType, handleGrammarExplain,
        handleSynonymFinder, handleAntonymFinder, handleDefineWords,
        handleWordPower, handleVocabComplexity, handleJargonSimplifier,
        handleFormalityDetector, handleClicheDetector,
        // New Generator AI handlers
        handleRegexGen, handleWritingPrompt, handleTeamNameGen, handleMockApiResponse,
