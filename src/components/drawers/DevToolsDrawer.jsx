...
export function JsonPathDrawer({ text, onResult, showAlert }) {
    ...
    const handleQuery = () => {
        if (!text) { showAlert('Enter JSON in the editor first', 'warning'); return }
        try {
            ...
            for (const part of parts) {
                ...
                if (current === undefined) { onResult('JSON Path', 'No match found for path: ' + path); return }
            }
            onResult('JSON Path Result', JSON.stringify(current, null, 2))
            showAlert('JSON path query executed', 'success')
        } catch { showAlert('Invalid JSON or path expression', 'danger') }
    }
            ...
            <div className="tu-gen-card">
                <div className="tu-gen-section">
                    ...
                    <input
                        ...
                        style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg-2)', color: 'var(--text-1)', fontFamily: 'var(--font-mono)' }}
                    />
                ...
                <button
                    ...
                    onClick={handleQuery}
                    style={{ width: '100%', marginTop: '12px', padding: '10px', borderRadius: '8px', background: 'var(--green)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}
                >
                    Query
                </button>
            </div>
...
export function SampleJsonDrawer({ onResult, showAlert }) {
    ...
    const templates = {
        user: { id: 1, name: 'John Doe', email: 'john@example.com', age: 30, role: 'admin', active: true, created_at: new Date().toISOString() },
        product: { id: 101, name: 'Wireless Headphones', price: 79.99, category: 'Electronics', in_stock: true, rating: 4.5, tags: ['audio', 'wireless', 'bluetooth'] },
        order: { id: 'ORD-001', customer_id: 1, items: [{ product_id: 101, quantity: 2, price: 79.99 }], total: 159.98, status: 'shipped', created_at: new Date().toISOString() },
        comment: { id: 1, post_id: 42, author: 'Jane', body: 'Great article!', likes: 15, created_at: new Date().toISOString() },
        api_response: { status: 200, data: [{ id: 1, name: 'Item 1' }, { id: 2, name: 'Item 2' }], pagination: { page: 1, per_page: 10, total: 42, total_pages: 5 }, meta: { request_id: 'abc-123' } },
    }
