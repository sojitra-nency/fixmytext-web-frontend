...
describe('useGamification', () => {
  ...
  it('recordToolUse increments totalOps and xp', () => {
    const { result } = renderHook(() => useGamification())
    act(() => { result.current.recordToolUse('uppercase', 10) })
  ...
  it('recordToolUse grants bonus XP for new tool discovery', () => {
    const { result } = renderHook(() => useGamification())
    act(() => { result.current.recordToolUse('uppercase', 5) })
  ...
  it('setPersona updates persona state', () => {
    const { result } = renderHook(() => useGamification())
    act(() => { result.current.setPersona('developer') })
  ...
  it('setPersona syncs to API when authenticated', () => {
    const { result } = renderHook(() => useGamification())
    act(() => { result.current.setPersona('writer') })
  ...
  it('loads state from localStorage on init', () => {
    localStorage.setItem('fmx_gamification', JSON.stringify({
      totalOps: 5,
      xp: 50,
