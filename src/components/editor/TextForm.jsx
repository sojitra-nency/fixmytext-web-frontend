...
import { JsonPathDrawer, MarkdownPreviewDrawer, LoremIpsumDrawer, SampleJsonDrawer } from '../drawers/DevToolsDrawer'
...
export default function TextForm(props) {
    ...
    const handleColorConvert = () => {
        ...
        } else { showAlert('Enter a HEX (#FF5733) or RGB (rgb(255,87,51)) color', 'danger'); return }
    ...
    const handleCronExplain = () => {
        ...
        if (parts.length < 5 || parts.length > 6) { showAlert('Enter a valid cron expression (5 or 6 fields)', 'danger'); return }
        ...
        if (dow !== '*') {
            ...
            const dayStr = dow.split(',').map(d => {
                if (d.includes('-')) { const [s, e] = d.split('-'); return `${days[s] || s} through ${days[e] || e}` }
                return days[d] || d
            }).join(', ')
    ...
    const handleReadingLevel = () => {
        ...
        const syllables = words.reduce((sum, w) => {
            const s = w.toLowerCase().replace(/(?:[^laeiouy]es|ed|[^laeiouy]e)$/, '').match(/[aeiouy]{1,2}/g)
    ...
    const handleOverusedWords = () => {
        const t = textRef.current
        if (!t) return
        const stopWords = new Set(['the','a','an','and','or','but','in','on','at','to','for','of','with','by','is','it','as','was','were','be','been','being','have','has','had','do','does','did','will','would','could','should','may','might','can','this','that','these','those','i','you','he','she','we','they','me','him','her','us','them','my','your','his','its','our','their','not','no','so','if'])
        const words = t.toLowerCase().match(/\b[a-z]+\b/g) || []
        const freq = {}
        words.filter(w => !stopWords.has(w) && w.length > 2).forEach(w => { freq[w] = (freq[w] || 0) + 1 })
        const total = Object.values(freq).reduce((a, b) => a + b, 0)
        const overused = Object.entries(freq).filter(([, c]) => (c / total) > 0.03).sort((a, b) => b[1] - a[1])
    ...
    const handleNumToWords = () => {
        ...
        const convert = (n) => {
            ...
            let result = ''
            if (n >= 1e9) { result += convert(Math.floor(n / 1e9)) + ' billion '; n %= 1e9 }
            if (n >= 1e6) { result += convert(Math.floor(n / 1e6)) + ' million '; n %= 1e6 }
    ...
    const handleTextToTable = () => {
        ...
        if (rows.length < 2) { showAlert('Need at least a header + 1 data row', 'danger'); return }
    ...
    const handlePlaceholderImg = () => {
        ...
        const w = match ? match[1] : '800', h = match ? match[2] : '600'
        const urls = [
    ...
    const handlerMap = useMemo(() => ({
        ...
        handleFormatSql, handleFormatXml, handleJsonMinify, handleJsonToTs,
        handleUuidGen, handleTimestampConvert, handleColorConvert,
        handleUlidGen, handleCronExplain, handleHttpHeaderParse, handleUrlParser,
        ...
        handleNanoidGen, handleTimestampGen, handleUsernameGen, handlePlaceholderImg,
        ...
        handleReadingTime, handleCharCount, handleTextStats,
        handleDuplicateWords, handleOverusedWords,
        handleNumToWords, handleWordsToNum,
        handleDateFormat: ai.handleDateFormat,
        handleRomanNumeral,
        handleQrFromText, handleMdToHtml, handleTextToTable,
        handleExtractEmails, handleExtractUrls, handleExtractNumbers,
    ...
    const renderDrawerContent = () => {
        switch (activePanel) {
            ...
            case 'cipherdrawer': return null // Renders inline
            case 'diffdrawer': return null // Renders inline
            case 'fakedata': return null // Renders inline
            case 'jsonpath': return null // Renders inline
            case 'mdpreview': return null // Renders inline
            case 'loremipsum': return null // Renders inline
            case 'samplejson': return null // Renders inline
            default: return null
        }
    }
            ...
            <div className="tu-forge-center">
                ...
                <div ref={splitRef} className="tu-editor-split" style={{ gridTemplateColumns: `${splitResize.size}fr 4px ${100 - splitResize.size}fr` }}>
                    ...
                    <div className={`tu-editor-input${workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId === 'compare' ? ' tu-editor-input--split' : ''}`}>
                        {['password', 'randtext', 'fakedata', 'loremipsum', 'samplejson'].includes(workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId) ? (
                            <div className="tu-gen-fullpage">
                                {workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId === 'password'
                                    ...
                                    : workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId === 'fakedata'
                                    ? <FakeDataDrawer
                                        activeTool={workspaceTabs.find(t => t.id === activeWorkspaceId)?.tool}
                                        onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                        showAlert={showAlert}
                                      />
                                    : workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId === 'loremipsum'
                                    ? <LoremIpsumDrawer
                                        onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                        showAlert={showAlert}
                                      />
                                    : workspaceTabs.find(t => t.id === activeWorkspaceId)?.panelId === 'samplejson'
                                    ? <SampleJsonDrawer
                                        onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                        showAlert={showAlert}
                                      />
                        ...
                        {(() => {
                            ...
                            switch (panelId) {
                                ...
                                case 'cipherdrawer': return <CipherDrawer
                                    activeTool={workspaceTabs.find(t => t.id === activeWorkspaceId)?.tool}
                                    text={text}
                                    onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                    showAlert={showAlert}
                                ...
                                />
                                case 'diffdrawer': return <DiffDrawer
                                    activeTool={workspaceTabs.find(t => t.id === activeWorkspaceId)?.tool}
                                    text={text}
                                    onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                    showAlert={showAlert}
                                />
                                case 'jsonpath': return <JsonPathDrawer
                                    text={text}
                                    onResult={(label, result) => { ai.setAiResult({ label, result }); setPreviewMode('result') }}
                                    showAlert={showAlert}
                                />
                                case 'mdpreview': return <MarkdownPreviewDrawer text={text} />
                                default: return null
                            }
                        })()}
                    ...
                    <div className="tu-editor-output">
                        {(() => {
                            ...
                            if (ws?.type === 'drawer') {
                                ...
                                if (['find', 'password', 'randtext', 'wraplines', 'filterlines', 'droplines', 'truncatelines', 'nthlines', 'cipherdrawer', 'diffdrawer', 'jsonpath', 'mdpreview', 'fakedata', 'loremipsum', 'samplejson'].includes(ws.panelId)) {
                            ...
                            const isNoInputDrawer = ['password', 'randtext', 'fakedata', 'loremipsum', 'samplejson'].includes(ws?.panelId)
