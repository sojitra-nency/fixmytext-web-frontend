...
export default function DiffDrawer({ activeTool, text, onResult, showAlert }) {
  ...
  const handleCompare = () => {
    if (!text || !textB) {
      showAlert('Enter text in both sides', 'warning');
      return;
    }
    ...
    switch (toolId) {
      ...
      case 'json_diff': {
        try {
          ...
          showAlert('Both texts must be valid JSON', 'danger');
        }
        break;
      }
      case 'list_diff': {
      ...
      case 'text_overlap': {
        ...
        onResult(
          'Text Overlap',
          filtered.length
            ? `Found ${filtered.length} shared phrase(s):\n\n${filtered
                .slice(0, 20)
                .map((p) => `• "${p}"`)
                .join('\n')}`
  ...
  const titles = {
    char_diff: 'Character Diff',
    word_diff: 'Word Diff',
    similarity_pct: 'Similarity Score',
    json_diff: 'JSON Diff',
    list_diff: 'List Diff',
    text_overlap: 'Overlap Detector',
  };
