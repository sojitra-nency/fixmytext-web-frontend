...
vi.mock('../../store/api/textApi', () => ({
  useTransformTextMutation: () => [vi.fn().mockResolvedValue({ result: 'transformed' }), { isLoading: false }],
}))
...
vi.mock('../../hooks/useFindReplace', () => ({
  default: () => ({
    findText: '', setFindText: vi.fn(), replaceText: '', setReplaceText: vi.fn(),
    handleFindReplace: vi.fn(), handleReplaceAll: vi.fn(), matchCount: 0,
  }),
}))
vi.mock('../../hooks/useTextCompare', () => ({
  default: () => ({
    compareText: '', setCompareText: vi.fn(), diffResult: null, setDiffResult: vi.fn(),
    handleCompare: vi.fn(),
  }),
}))
vi.mock('../../hooks/useGenerators', () => ({
  default: () => ({
    handleGenerateRandom: vi.fn(), handleGeneratePassword: vi.fn(),
    passwordLength: 16, setPasswordLength: vi.fn(),
    includeUpper: true, setIncludeUpper: vi.fn(),
    includeLower: true, setIncludeLower: vi.fn(),
    includeNumbers: true, setIncludeNumbers: vi.fn(),
    includeSymbols: false, setIncludeSymbols: vi.fn(),
  }),
}))
vi.mock('../../hooks/useFormatter', () => ({
  default: () => ({
    handleFormatHtml: vi.fn(), handleFormatCss: vi.fn(),
    handleFormatJs: vi.fn(), handleFormatTs: vi.fn(),
...
vi.mock('../../hooks/useAiTools', () => ({
  default: () => ({
    aiResult: null, setAiResult: vi.fn(), hasMarkdown: vi.fn(() => false),
    handleFixGrammar: vi.fn(), handleParaphrase: vi.fn(), handleProofread: vi.fn(),
    handleSummarize: vi.fn(), handleEli5: vi.fn(), handleLengthenText: vi.fn(),
    handleEmailRewrite: vi.fn(), handleTweetShorten: vi.fn(), handleHashtags: vi.fn(),
    handleSeoTitles: vi.fn(), handleMetaDescriptions: vi.fn(), handleBlogOutline: vi.fn(),
    handleKeywords: vi.fn(), handleSentiment: vi.fn(), handleGenerateTitle: vi.fn(),
    handleRefactorPrompt: vi.fn(), handleEmojify: vi.fn(), handleChangeFormat: vi.fn(),
    handleChangeTone: vi.fn(), handleTranslate: vi.fn(), handleTransliterate: vi.fn(),
    handleSplitToLines: vi.fn(), handleJoinLines: vi.fn(), handlePadLines: vi.fn(),
    handleCaesarCipher: vi.fn(), handleRailFenceEnc: vi.fn(), handleRailFenceDec: vi.fn(),
    handleAcademicStyle: vi.fn(), handleCreativeStyle: vi.fn(), handleTechnicalStyle: vi.fn(),
    handleActiveVoice: vi.fn(), handleRedundancyRemover: vi.fn(), handleSentenceSplitter: vi.fn(),
    handleConciseness: vi.fn(), handleResumeBullets: vi.fn(), handleMeetingNotes: vi.fn(),
    handleCoverLetter: vi.fn(), handleOutlineToDraft: vi.fn(), handleContinueWriting: vi.fn(),
    handleRewriteUnique: vi.fn(), handleToneAnalyzer: vi.fn(),
    handleLinkedinPost: vi.fn(), handleTwitterThread: vi.fn(), handleInstagramCaption: vi.fn(),
    handleYoutubeDesc: vi.fn(), handleSocialBio: vi.fn(), handleProductDesc: vi.fn(),
    handleCtaGenerator: vi.fn(), handleAdCopy: vi.fn(), handleLandingHeadline: vi.fn(),
    handleEmailSubject: vi.fn(), handleContentIdeas: vi.fn(), handleHookGenerator: vi.fn(),
    handleAngleGenerator: vi.fn(), handleFaqSchema: vi.fn(),
    handlePosTagger: vi.fn(), handleSentenceType: vi.fn(), handleGrammarExplain: vi.fn(),
    handleSynonymFinder: vi.fn(), handleAntonymFinder: vi.fn(), handleDefineWords: vi.fn(),
    handleWordPower: vi.fn(), handleVocabComplexity: vi.fn(), handleJargonSimplifier: vi.fn(),
    handleFormalityDetector: vi.fn(), handleClicheDetector: vi.fn(),
    handleRegexGen: vi.fn(), handleWritingPrompt: vi.fn(), handleTeamNameGen: vi.fn(),
    handleMockApiResponse: vi.fn(), handleDateFormat: vi.fn(),
    handleCurlToCode: vi.fn(),
    handleAiDismiss: vi.fn(), handleDetectLanguage: vi.fn(),
    autoDetectLang: false, setAutoDetectLang: vi.fn(),
    detectedLang: null, setDetectedLang: vi.fn(),
    selectedTone: 'professional', setSelectedTone: vi.fn(),
    selectedFormat: 'markdown', setSelectedFormat: vi.fn(),
    selectedLanguage: 'Spanish', setSelectedLanguage: vi.fn(),
    selectedTranslit: 'latin', setSelectedTranslit: vi.fn(),
    selectKey: null, setterKey: null,
  }),
}))
vi.mock('../../hooks/useSpeech', () => ({
  default: () => ({
    handleTts: vi.fn(), handleSpeechToText: vi.fn(),
    listening: false,
  }),
}))
vi.mock('../../hooks/useExport', () => ({
  default: () => ({
    handleDownloadTxt: vi.fn(), handleDownloadPdf: vi.fn(),
    handleDownloadDocx: vi.fn(), handleDownloadJson: vi.fn(),
    handleDownloadCsv: vi.fn(), handleDownloadMd: vi.fn(),
    setOutputText: vi.fn(),
  }),
}))
vi.mock('../../hooks/useRegexTester', () => ({
  default: () => ({
    pattern: '', setPattern: vi.fn(), flags: '', setFlags: vi.fn(),
    matches: [], matchCount: 0, handleTest: vi.fn(),
  }),
}))
vi.mock('../../hooks/useTemplates', () => ({
  default: () => ({
    templates: [], templateName: '', setTemplateName: vi.fn(),
    handleSaveTemplate: vi.fn(), handleLoadTemplate: vi.fn(), handleDeleteTemplate: vi.fn(),
    saveDirectly: vi.fn(),
  }),
}))
vi.mock('../../hooks/useHistory', () => ({
  default: () => ({
    history: [], pushHistory: vi.fn(), handleUndo: vi.fn(), handleRedo: vi.fn(),
    handleClearHistory: vi.fn(), canUndo: false, canRedo: false,
  }),
}))
...
vi.mock('../../hooks/usePipeline', () => ({
  default: () => ({
    steps: [], addStep: vi.fn(), clearSteps: vi.fn(),
  }),
}))
vi.mock('../../hooks/useSmartSuggestions', () => ({
  default: () => ({
    suggestions: [], dismiss: vi.fn(),
  }),
}))
vi.mock('../../hooks/useToolSearch', () => ({
  default: () => ({
    query: '', setQuery: vi.fn(), results: [], isOpen: false,
    open: vi.fn(), close: vi.fn(),
  }),
}))
vi.mock('../../hooks/useResize', () => ({
  default: (dir, defaultSize) => ({
    size: defaultSize, setSize: vi.fn(), onMouseDown: vi.fn(),
  }),
}))
vi.mock('../../hooks/useTrialLimit', () => ({
  default: () => ({
    checkTrial: vi.fn(() => true), showSignInGate: false, dismissGate: vi.fn(),
    trialCount: 0,
  }),
}))
vi.mock('../../hooks/useKeyboardShortcuts', () => ({
  default: () => ({
    shortcutsOpen: false, setShortcutsOpen: vi.fn(),
    groups: [], overrides: {}, updateBinding: vi.fn(),
    resetAll: vi.fn(), resetOne: vi.fn(), isCustomized: vi.fn(() => false),
  }),
}))

// ── Child component mocks ──
vi.mock('./ToolPanel', () => ({
  default: ({ onToolClick, tools }) => React.createElement('div', {
...
vi.mock('./OutputPanel', () => ({
  default: (props) => React.createElement('div', { 'data-testid': 'output-panel' }, `OutputPanel:${props.previewMode || 'none'}`),
}))
vi.mock('../drawers/DrawerPanel', () => ({
  default: ({ children, title }) => React.createElement('div', { 'data-testid': 'drawer-panel' }, title, children),
}))
...
describe('TextForm', () => {
  ...
  it('shows Sign Out option in settings menu for authenticated users', () => {
    const props = { ...defaultProps, isAuthenticated: true, user: { display_name: 'Alice', email: 'alice@example.com' } }
    render(<TextForm {...props} />)
    const avatarBtn = document.querySelector('.tu-activity-avatar')
    fireEvent.click(avatarBtn)
  ...
  it('shows the correct user letter in avatar when authenticated', () => {
    const props = { ...defaultProps, isAuthenticated: true, user: { display_name: 'Bob', email: 'bob@example.com' } }
    render(<TextForm {...props} />)
  ...
  it('shows sign-in gate modal when trial.showSignInGate is true', () => {
    vi.doMock('../../hooks/useTrialLimit', () => ({
      default: () => ({
        checkTrial: vi.fn(() => true), showSignInGate: true, dismissGate: vi.fn(), trialCount: 3,
      }),
    }))
  ...
  it('shows PRO badge when subscription.isPro is true', () => {
    const props = {
      ...
      subscription: { isPro: true, checkToolAccess: vi.fn(() => true), refetchStatus: vi.fn(), totalCredits: 0 },
    }
    render(<TextForm {...props} />)
  ...
  it('shows credits count when authenticated and not pro', () => {
    const props = {
      ...
      subscription: { isPro: false, checkToolAccess: vi.fn(() => true), refetchStatus: vi.fn(), totalCredits: 25 },
    }
    render(<TextForm {...props} />)
  ...
  it('shows Upgrade to Pro button in settings for non-pro authenticated users', () => {
    const props = {
      ...
      isAuthenticated: true,
      user: { display_name: 'Free User', email: 'free@example.com' },
      subscription: { isPro: false, checkToolAccess: vi.fn(() => true), refetchStatus: vi.fn(), totalCredits: 5 },
    }
    render(<TextForm {...props} />)
    const avatarBtn = document.querySelector('.tu-activity-avatar')
    fireEvent.click(avatarBtn)
