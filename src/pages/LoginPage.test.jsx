...
describe('LoginPage', () => {
  ...
  it('submits form successfully', async () => {
    ...
    await waitFor(() => {
      expect(mockLogin).toHaveBeenCalledWith({ email: 'test@example.com', password: 'pass1234', remember_me: false })
  ...
  it('shows generic error when no detail', async () => {
    ...
    fireEvent.submit(screen.getByText('Sign In').closest('form'))
    await waitFor(() => {
      expect(showAlert).toHaveBeenCalledWith('Login failed. Please check your credentials.', 'danger')
    })
  })
})
