    ...
    <div className="tu-find-inputs">
            ...
            <div className="tu-diff-output" style={{ maxHeight: 300, marginTop: '0.5rem' }}>
              {[...history].reverse().map((h, ri) => {
                ...
                return (
                  <div
                    key={i}
                    style={{ borderBottom: '1px solid var(--border)', padding: '0.5rem 0.75rem' }}
                  >
                    <div
                      style={{
                        display: 'flex',
                    ...
                    >
                      <span
                        style={{ fontWeight: 700, fontSize: '0.78rem', color: 'var(--violet)' }}
                      >
                        {h.operation}
                      </span>
                      <span style={{ flex: 1 }} />
                    ...
                    <div
                      style={{ fontSize: '0.72rem', color: 'var(--fg2)', marginBottom: '0.2rem' }}
                    >
                      <b>In:</b> {truncate(h.original)}
                    </div>
                    <div
                      style={{ fontSize: '0.72rem', color: 'var(--fg2)', marginBottom: '0.35rem' }}
                    >
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                      <button
                        className="tu-btn tu-btn--tools"
                        style={{ fontSize: '0.65rem', padding: '0.12rem 0.45rem' }}
                        onClick={() => handleRestoreOriginal(i)}
                      >
                        Restore Input
                      </button>
                      <button
      ...
      {view === 'saved' && accessToken && (
        <>
          ...
          >
            <span style={{ fontSize: '0.72rem', color: 'var(--fg2)' }}>
              {serverHistory?.total || 0} total operation
              {(serverHistory?.total || 0) !== 1 ? 's' : ''}
            </span>
            <span style={{ flex: 1 }} />
            <button
              className="tu-btn tu-btn--tools"
              ...
              disabled={!serverHistory || serverHistory.total === 0}
            >
              Clear All
            </button>
          </div>
              ...
              {serverHistory.items.map((h) => (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.72rem', color: 'var(--fg2)', marginBottom: '0.2rem' }}>
                    <b>In:</b> {truncate(h.input_preview)}{' '}
                    <span style={{ fontSize: '0.6rem', color: 'var(--fg2)' }}>
                      ({h.input_length} chars)
                    </span>
                  </div>
                  <div
                    style={{ fontSize: '0.72rem', color: 'var(--fg2)', marginBottom: '0.35rem' }}
                  >
                    <b>Out:</b> {truncate(h.output_preview)}{' '}
                    <span style={{ fontSize: '0.6rem', color: 'var(--fg2)' }}>
                      ({h.output_length} chars)
                    </span>
                  </div>
                  <div style={{ display: 'flex', gap: '0.4rem' }}>
                    <button
