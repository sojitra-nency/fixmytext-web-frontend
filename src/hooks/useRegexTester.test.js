...
describe('useRegexTester', () => {
  ...
  it('shows alert when pattern is empty', () => {
    const { result } = renderHook(() => useRegexTester('text', showAlert))
    act(() => { result.current.handleRegexTest() })
  ...
  it('shows alert when text is empty', () => {
    ...
    act(() => { result.current.setRegexPattern('\\w+') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('finds global matches', () => {
    ...
    act(() => { result.current.setRegexPattern('hello') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('finds non-global match', () => {
    ...
    act(() => { result.current.setRegexPattern('world') })
    act(() => { result.current.setRegexFlags('') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('reports 0 matches with info', () => {
    ...
    act(() => { result.current.setRegexPattern('xyz') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('shows alert for invalid regex', () => {
    const { result } = renderHook(() => useRegexTester('text', showAlert))
    act(() => { result.current.setRegexPattern('[bad') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('captures groups', () => {
    ...
    act(() => { result.current.setRegexPattern('(\\d{4})-(\\d{2})-(\\d{2})') })
    act(() => { result.current.handleRegexTest() })
  ...
  it('handles zero-length matches (advances lastIndex)', () => {
    ...
    act(() => { result.current.setRegexPattern('(?=a)') })
    act(() => { result.current.setRegexFlags('g') })
