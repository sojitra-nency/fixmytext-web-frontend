  ...
  return (
    <div className="tu-compare-input">
      <div className="tu-editor-topbar">
        <span className="tu-editor-label" title="~/FixMyText/workspace/compare-with.txt">
          COMPARE WITH
        </span>
...
export default function CompareOutput({ diffResult, compareText }) {
  ...
  const buildInlineNums = () => {
    ...
    return diffResult.map((d) => {
      ...
      if (d.type === 'removed') {
        leftNum++;
        return { left: leftNum, right: null };
      }
      rightNum++;
      return { left: null, right: rightNum };
    });
  };
  ...
  const buildSideLines = () => {
    ...
    for (const d of diffResult) {
      if (d.type === 'same') {
        ...
        rightNum++;
        lines.push({
          left: { num: leftNum, text: d.line, type: 'same' },
          right: { num: rightNum, text: d.line, type: 'same' },
        });
      } else if (d.type === 'removed') {
        leftNum++;
        lines.push({
          left: { num: leftNum, text: d.line, type: 'removed' },
        ...
        lines.push({
          ...
          right: { num: rightNum, text: d.line, type: 'added' },
        });
      }
    }
    return lines;
  ...
  if (!diffResult) {
    return (
      <div className="tu-compare-output">
        <div className="tu-editor-topbar">
          <span className="tu-editor-label" title="~/FixMyText/workspace/diff">
            DIFF OUTPUT
          </span>
          <div className="tu-topbar-stats">
            <span className="tu-topbar-stat">
  ...
  return (
    <div className="tu-compare-output">
      ...
      <div className="tu-editor-topbar">
        ...
        <div className="tu-topbar-stats">
          <span className="tu-topbar-stat tu-topbar-stat--added">
            <b>+{added}</b>
          </span>
          <span className="tu-topbar-stat tu-topbar-stat--removed">
            <b>−{removed}</b>
          </span>
          <span className="tu-topbar-stat">
            <b>{same}</b> same
      ...
      <div className="tu-diff-toolbar">
        ...
        <div className="tu-diff-controls">
          <button
            className={`tu-diff-view-btn${
              viewMode === 'inline' ? ' tu-diff-view-btn--active' : ''
            }`}
