...
describe('FmtConfigBar', () => {
  const defaultCfg = {
    tabWidth: 2, useTabs: false, semi: true, singleQuote: false,
    trailingComma: 'es5', bracketSpacing: true, arrowParens: 'always',
    jsxSingleQuote: false, sortImports: true,
  }

  it('returns null for unknown toolId', () => {
    const { container } = render(<FmtConfigBar toolId="unknown" fmtCfg={defaultCfg} setFmtCfg={vi.fn()} />)
  ...
  it('renders HTML config bar with whitespace option', () => {
    render(<FmtConfigBar toolId="html_fmt" fmtCfg={{ ...defaultCfg, htmlWhitespaceSensitivity: 'css' }} setFmtCfg={vi.fn()} />)
  ...
  it('toggles bracket same line for HTML', () => {
    const setFmtCfg = vi.fn()
    render(<FmtConfigBar toolId="html_fmt" fmtCfg={{ ...defaultCfg, htmlWhitespaceSensitivity: 'css', bracketSameLine: false }} setFmtCfg={setFmtCfg} />)
  ...
  it('changes HTML whitespace sensitivity select', () => {
    const setFmtCfg = vi.fn()
    render(<FmtConfigBar toolId="html_fmt" fmtCfg={{ ...defaultCfg, htmlWhitespaceSensitivity: 'css' }} setFmtCfg={setFmtCfg} />)
    const selects = screen.getAllByRole('combobox')
