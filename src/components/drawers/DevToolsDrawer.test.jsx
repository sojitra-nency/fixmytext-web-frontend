...
import { JsonPathDrawer, MarkdownPreviewDrawer, LoremIpsumDrawer, SampleJsonDrawer } from './DevToolsDrawer'
...
describe('JsonPathDrawer', () => {
  ...
  it('queries valid JSON with simple path', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('$.store.book[0].title'), { target: { value: '$.name' } })
    fireEvent.click(screen.getByText('Query'))
  ...
  it('handles path with no match', () => {
    ...
    render(<JsonPathDrawer {...props} text={json} />)
    fireEvent.change(screen.getByPlaceholderText('$.store.book[0].title'), { target: { value: '$.missing' } })
    fireEvent.click(screen.getByText('Query'))
  ...
  it('handles array index in path', () => {
    ...
    render(<JsonPathDrawer {...props} text={json} />)
    fireEvent.change(screen.getByPlaceholderText('$.store.book[0].title'), { target: { value: '$.items[1]' } })
    fireEvent.click(screen.getByText('Query'))
  ...
  it('handles wildcard * path', () => {
    ...
    render(<JsonPathDrawer {...props} text={json} />)
    fireEvent.change(screen.getByPlaceholderText('$.store.book[0].title'), { target: { value: '$.*' } })
    fireEvent.click(screen.getByText('Query'))
...
describe('LoremIpsumDrawer', () => {
  ...
  it('generates paragraphs by default', () => {
    ...
    expect(props.onResult).toHaveBeenCalledWith('Lorem Ipsum', expect.stringContaining('Lorem ipsum'))
