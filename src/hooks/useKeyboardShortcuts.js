...
export default function useKeyboardShortcuts(actions) {
  ...
  const groups = useMemo(() => mergeBindings(DEFAULT_SHORTCUT_GROUPS, overrides), [overrides]);
  ...
  const updateBinding = useCallback(
    (id, binding) => {
      setOverrides((prev) => {
        ...
        if (isAuthenticated) {
          updateUiSettings({ keybindings: next })
            .unwrap()
            .catch(() => {});
        }
        return next;
