...
describe('useHistory', () => {
  ...
  it('pushHistory adds an entry', () => {
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Uppercase', 'hello', 'HELLO') })
  ...
  it('handleRestoreOriginal sets text to original', () => {
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Op', 'orig', 'res') })
    act(() => { result.current.handleRestoreOriginal(0) })
  ...
  it('handleRestoreResult sets text to result', () => {
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Op', 'orig', 'res') })
    act(() => { result.current.handleRestoreResult(0) })
  ...
  it('handleUndo restores original of last entry', () => {
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Op1', 'a', 'b') })
    act(() => { result.current.pushHistory('Op2', 'c', 'd') })
  ...
  it('handleRedo restores result of next entry', () => {
    ...
    act(() => { result.current.pushHistory('Op1', 'a', 'b') })
    act(() => { result.current.pushHistory('Op2', 'c', 'd') })
    act(() => { result.current.handleUndo() }) // undo Op2
    act(() => { result.current.handleUndo() }) // undo Op1
    act(() => { result.current.handleRedo() }) // redo Op1
  ...
  it('handleClearHistory empties history', () => {
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Op', 'a', 'b') })
  ...
  it('records operation to backend when authenticated', () => {
    ...
    const { result } = renderHook(() => useHistory(setText, showAlert))
    act(() => { result.current.pushHistory('Op', 'orig', 'res', { toolId: 'test', toolType: 'local' }) })
    expect(mockRecordOperation).toHaveBeenCalledWith(expect.objectContaining({
      tool_id: 'test',
      tool_type: 'local',
      status: 'success',
    }))
