        ...
        >
          ...
          <div className="tu-upgrade-usage">
            <div className="tu-upgrade-usage-label">
              <span>
                <strong>{blockedTool.label}</strong> usage today
              </span>
              <span className="tu-upgrade-usage-count">
          ...
          <div className="tu-pass-options">
            {singlePass && (
              <motion.div
                className={`tu-pass-option${
                  buyingId === 'day_single' ? ' tu-pass-option--loading' : ''
                }`}
                onClick={() =>
                  !buyingId && handleBuy(subscription.handleBuyPass, 'day_single', [blockedTool.id])
                }
                whileHover={{ y: -1 }}
              ...
              >
                ...
                <div className="tu-pass-option-body">
                  ...
                  <span className="tu-pass-option-desc">
                    {singlePass.uses_per_day} uses of {blockedTool.label} for 24h
                  </span>
                </div>
            ...
            {cheapestCredit && (
              <motion.div
                className={`tu-pass-option${
                  buyingId === cheapestCredit.id ? ' tu-pass-option--loading' : ''
                }`}
                onClick={() =>
                  !buyingId && handleBuy(subscription.handleBuyCredits, cheapestCredit.id)
                }
                whileHover={{ y: -1 }}
                whileTap={{ scale: 0.985 }}
              >
                ...
                <div className="tu-pass-option-body">
                  ...
                  <span className="tu-pass-option-desc">
                    {cheapestCredit.credits} credits, use on any tool, no expiry
                  </span>
                </div>
                <span className="tu-pass-option-price">
