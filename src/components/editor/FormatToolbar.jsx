...
function FormatToolbarPopup({ pos, onFormat, onClose }) {
  ...
  return createPortal(
    <div ref={ref} className="tu-fmt-toolbar" style={{ top: pos.top, left: pos.left }}>
      {FORMATS.map((fmt, i) => (
        <span key={fmt.key} className="tu-fmt-toolbar-group">
          ...
          <button className="tu-fmt-toolbar-btn" onClick={() => onFormat(fmt)} title={fmt.label}>
...
export default memo(function FormatToolbar({ textareaRef, text, setText, enabled }) {
  ...
  const checkSelection = useCallback(() => {
    ...
    const left = Math.max(
      rect.left + 20,
      Math.min(rect.right - 300, rect.left + rect.width / 2 - 150)
    );
