    ...
    <div className="tu-fmtbar">
      ...
      {spec.semi && (
        <Toggle
          ...
          label="Semicolons"
          tooltip="Add semicolons at the end of statements"
        />
      )}
      {spec.singleQuote && (
        <Toggle
          checked={fmtCfg.singleQuote}
          onChange={(v) => set({ singleQuote: v })}
          label="Single Quotes"
          tooltip="Use single quotes instead of double quotes"
        />
      )}
      {spec.bracketSpacing && (
        <Toggle
          checked={fmtCfg.bracketSpacing}
          onChange={(v) => set({ bracketSpacing: v })}
          label="Bracket Spacing"
          tooltip="Add spaces inside object braces: { foo } vs {foo}"
        />
      )}
      {spec.jsxSingleQuote && (
        <Toggle
          checked={fmtCfg.jsxSingleQuote}
          onChange={(v) => set({ jsxSingleQuote: v })}
          label="JSX Quotes"
          tooltip="Use single quotes in JSX attributes"
        />
      )}
      {spec.sortImports && (
        <Toggle
          checked={fmtCfg.sortImports}
          onChange={(v) => set({ sortImports: v })}
          label="Sort Imports"
          tooltip="Sort import statements alphabetically"
        />
      )}
      ...
      {spec.bracketSameLine && (
        <Toggle
          checked={fmtCfg.bracketSameLine}
          onChange={(v) => set({ bracketSameLine: v })}
          label="Bracket Same Line"
          tooltip="Put the > of a multi-line HTML element on the same line as the last attribute"
        />
      )}
      {spec.htmlWhitespace && (
        <div
          className="tu-fmtbar-field"
          title="How to handle whitespace in HTML: css (default), strict, or ignore"
        >
          <span className="tu-fmtbar-label">Whitespace</span>
          <select
            className="tu-fmtbar-select"
            value={fmtCfg.htmlWhitespaceSensitivity || 'css'}
            onChange={(e) => set({ htmlWhitespaceSensitivity: e.target.value })}
          >
      ...
      {spec.trailingComma && (
        <div
          className="tu-fmtbar-field"
          title="Add trailing commas: None, ES5 (objects/arrays), or All (including function params)"
        >
          <span className="tu-fmtbar-label">Trailing</span>
          <select
            className="tu-fmtbar-select"
            value={fmtCfg.trailingComma}
            onChange={(e) => set({ trailingComma: e.target.value })}
          >
      ...
      {spec.arrowParens && (
        <div
          className="tu-fmtbar-field"
          title="Parentheses around a single arrow function parameter: (x) => x vs x => x"
        >
          <span className="tu-fmtbar-label">Arrows</span>
          <select
            className="tu-fmtbar-select"
            value={fmtCfg.arrowParens}
            onChange={(e) => set({ arrowParens: e.target.value })}
          >
